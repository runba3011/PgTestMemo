...
public abstract class Player {
  ...
  protected int agi;
  /** プレイヤーの状態異常 */
  private ArrayList<SpecialEffect> specialEffectList = new ArrayList<>();
  ...
  protected static class StatusRange {
    private int minHp;
    private int maxHp;
    private int minMp;
    private int maxMp;
  ...
  public int getStatus(String name, GenerateNumberIndex index, StatusRange statusRange) {
    ...
    int randomValue = (int) (randomRangeMax
        * ((double) generateNumber(name, index.ordinal()) / GENERATE_NUMBER_MAX));
  ...
  protected boolean isCritical() {
    return random.nextInt(GENERATE_NUMBER_MAX) <= this.luck;
  }
  ...
  public void receiveSpecialEffectDamage() {
    for (SpecialEffect effect : this.specialEffectList) {
      ...
      if(0 < damage){
        System.out.println(
            String.format("%s は %s により %d のダメージを受けた！", this.name, effectNum.toString(), damage));
      }
      else{
        //受けるダメージ量が負の値、つまり回復する状態異常の場合は回復した旨の内容を出力する。
        System.out.println(
          String.format("%s は %s により %d 回復した！！", this.name, effectNum.toString(), Math.abs(damage)));
